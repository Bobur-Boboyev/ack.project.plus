...
class AnalyticsService:
    ...
    def get_manager_dashboard(self, current_user: User):
        ...
        active_projects = sum(
            1
            for p in self.repo.get_manager_projects(manager_id)
            if p.status == "active"
        )
        ...
        return {
            ...
            "unread_notifications": unread_notifications,
            # Yangi
            "project_breakdown": self.repo.get_manager_project_status_breakdown(
                manager_id
            ),
            ...
            "alerts": self._build_manager_alerts(
                manager_id, active_workers, workers_reported
            ),
            "recent_reports": recent_reports,
        }

    def _build_manager_alerts(
        self, manager_id: int, active_workers: int, workers_reported: int
    ) -> list:
        alerts = []
    ...
    def _build_worker_alerts(
        self, user_id: int, overdue_tasks: int, blocked_tasks: int
    ) -> list:
        alerts = []
